...
- **⚠️ [LiffLayout.test.js](file:///Users/user/Desktop/playgroud/playground/playground-frontend/src/layouts/LiffLayout.test.js) มี 4 เทส Fail Pre-existing**: `mount(LiffLayout, { global: { plugins: [router] } })` ไม่ได้ใส่ Pinia Plugin เลย ทำให้ `useAuthStore()`/`useFeatureStore()` โยน `"getActivePinia() was called but there was no active Pinia"` ทันทีตอน Mount — พบระหว่างทำ Phase 17 ไม่เกี่ยวกับโค้ดที่แก้ ยังไม่ได้แก้ (ต้องเพิ่ม `createTestingPinia()` เข้า `global.plugins`)
