...
- **LIFF Client Redirect Guard** ([router/index.js](file:///Users/user/Desktop/playgroud/playground/playground-frontend/src/router/index.js)): `liffNavigationGuard` เช็ค `liff.isInClient()` ก่อนทุกหน้า `/liff/*` — ถ้าไม่ได้เปิดผ่านแอป LINE จริง จะ `window.location.href` เด้งไป `https://liff.line.me/<LIFF_ID><path เดิม>` ให้อัตโนมัติ (ปิดใน `import.meta.env.DEV` เพื่อไม่รบกวนการทดสอบผ่าน Browser, เว้น `/web/login`)
