      ...
      <div v-else class="flex flex-col items-center text-center space-y-3">
        ...
        <div class="space-y-1">
          <h1 class="text-lg font-black tracking-tight text-white flex items-center justify-center gap-1.5">
            <span>สวัสดี, {{ profile?.displayName || 'ลูกบ้าน' }}</span>
          </h1>
          <p class="text-xs text-slate-400 font-medium">
            กรุณาระบุรหัส PIN 6 หลักเพื่อเข้าใช้งาน
          </p>
        </div>
      </div>
    </header>
...
// Haptic vibration feedback (ถ้าบราวเซอร์รองรับ)
...
/**
 * =========================================================
 * Auto-Submit Logic: Watcher จับค่า enteredPin ครบ 6 หลัก
 * =========================================================
 */
...
const submitPinLogin = async () => {
  ...
  try {
    ...
    const res = await authStore.loginPin(idToken, enteredPin.value);
    ...
    if (res?.success) {
      ...
      triggerErrorFeedback(res?.message || 'รหัส PIN ไม่ถูกต้อง');
    }
  } catch (err) {
...
const triggerErrorFeedback = (msg) => {
  ...
  // Shake animation 600ms แล้วเคลียร์ค่า PIN เพื่อให้กดใหม่
  setTimeout(() => {
...
// รองรับการกดแป้นพิมพ์จริง (Physical Keyboard)
...
onMounted(async () => {
  ...
  try {
    ...
    if (isLiffLoggedIn()) {
      ...
      // Mock profile เมื่อรันในบราวเซอร์ทั่วไป
